  ...
  return (
    <div className="space-y-8">
      {stalls.map((stall) => (
        <div
          key={stall.id}
          className="border border-green-800/50 rounded-lg bg-black/30 overflow-hidden"
        >
          <div className="p-4 bg-gray-900/50 border-b border-green-800/50">
            <h3 className="text-2xl font-headline text-green-300">
              Puesto {stall.number}
            </h3>
            <p className="text-green-500 text-sm">{stall.name}</p>
          </div>
